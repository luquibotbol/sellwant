...
// ------------------------------------------------------------------- email
//
// Notifications. The database queues them (20260821_email_outbox.sql); this
// drains the queue on a cron and hands each one to Resend.
//
// Resend rather than the Cloudflare SMTP that already carries Supabase's auth
// mail, for one blunt reason: a Worker cannot speak SMTP. Port 25 is blocked
// outbound and hand-rolling authenticated SMTP over a socket to send "you have
// an offer" is not a thing anyone should maintain. Anything sending from
// inside the Worker needs an HTTP API.

/** Where an email's links point. There is no request to read an origin from
 *  on a scheduled run, and this is the only host the app is ever served on. */
const SITE = `https://${CANONICAL}`;

/**
 * The unsubscribe proof.
 *
 * Derived, not stored. The obvious place for a token is a column on profiles,
 * and profiles is readable by every signed-in account -- so that column would
 * hand anyone the ability to unsubscribe anyone. An HMAC over the user id has
 * nothing to leak and needs no migration.
 *
 * 128 bits of it, because this guards a preference rather than an account and
 * the whole thing has to survive being wrapped by a mail client.
 */
export async function unsubSig(secret, profileId, kind) {
  const enc = new TextEncoder();
  const key = await crypto.subtle.importKey(
    'raw', enc.encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
  const mac = await crypto.subtle.sign('HMAC', key, enc.encode(`${profileId}:${kind}`));
 ...
 *
 * Empty never matches, including empty against empty. Nothing reaches this
 * with a blank computed signature today -- unsubSig always returns 32 hex
 * characters -- but "both sides absent" resolving to "authorised" is the wrong
 * default to leave sitting under an unsubscribe link.
 */
...
const unsubUrl = (profileId, kind, sig) =>
  `${SITE}/api/unsubscribe?u=${encodeURIComponent(profileId)}` +
  `&k=${encodeURIComponent(kind)}&s=${sig}`;

/** One line, used by both bodies, so the HTML and the plain text cannot drift
 *  apart on the one sentence that must always be there. */
const HOLDS_NOTHING =
  'Nothing is agreed until you accept, and SellWant never holds the ticket or the money.';

/**
 * One "you have an offer" email, as subject + both bodies.
 *
 * Exported and pure so the wording can be tested without a network: every
 * other way of checking an email involves sending one.
 *
 * The copy stays away from anything that sounds like we are holding the
 * ticket or the money. We are telling someone a number a stranger said out
 * loud, and nothing else has happened yet.
 */
export function offerEmail(row, unsub) {
  const p = row.payload ?? {};
  const n = Math.max(1, row.collapsed ?? 1);
  const amount = money(p.amount_cents ?? 0);
  const title = plain(p.listing_title, 80) || 'your listing';
  const link = `${SITE}/event/${row.listing_id}`;

  // A counter is an answer to something you said, so it reads as a reply.
  // A first offer is news about your listing. Same row, different sentence.
  const subject = p.is_counter
    ? `${amount} back on “${title}”`
    : n > 1
      ? `${n} new offers on “${title}” — latest ${amount}`
      // "latest", not "up to": the batch keeps the newest offer, and we do not
      // know it is the highest. Saying so would be a number we made up.
      : `${amount} offered on “${title}”`;

  const lead = p.is_counter
    ? `Your offer on <strong>${esc(title)}</strong> was countered at <strong>${esc(amount)}</strong>.`
    : n > 1
      ? `You have <strong>${n} new offers</strong> on <strong>${esc(title)}</strong>. The most recent is <strong>${esc(amount)}</strong>.`
      : `Someone offered <strong>${esc(amount)}</strong> on <strong>${esc(title)}</strong>.`;

  const note = p.message
    ? `<p style="margin:0 0 20px;padding:12px 14px;background:#f4f4f5;border-radius:8px;` +
      `font-size:15px;line-height:1.5;color:#3f3f46">${esc(plain(p.message, 300))}</p>`
    : '';

  // Sentences stay on one source line each. Wrapping them reads better here and
  // renders identically -- but it puts a newline in the middle of the copy, so
  // the one line that must never change silently becomes untestable.
  const FONT = "-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif";
  const html =
    `<!doctype html><html><body style="margin:0;padding:24px;background:#fafafa;font-family:${FONT}">` +
    `<div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:28px;border:1px solid #e4e4e7">` +
    `<p style="margin:0 0 16px;font-size:17px;line-height:1.5;color:#18181b">${lead}</p>` +
    note +
    `<a href="${esc(link)}" style="display:inline-block;background:#18181b;color:#fff;` +
    `text-decoration:none;padding:12px 20px;border-radius:8px;font-size:15px;font-weight:600">See the offer</a>` +
    `<p style="margin:24px 0 0;font-size:13px;line-height:1.5;color:#71717a">${HOLDS_NOTHING}</p>` +
    `</div>` +
    `<p style="max-width:480px;margin:16px auto 0;font-size:12px;color:#a1a1aa;text-align:center">` +
    `<a href="${esc(unsub)}" style="color:#a1a1aa">Stop emails about offers</a></p>` +
    `</body></html>`;

  // Sent alongside the HTML, not as a fallback nobody reads: a plain part is
  // most of what a spam filter can actually parse, and this domain is starting
  // from no sending reputation at all.
  const text =
    (p.is_counter
      ? `Your offer on "${title}" was countered at ${amount}.`
      : n > 1
        ? `You have ${n} new offers on "${title}". The most recent is ${amount}.`
        : `Someone offered ${amount} on "${title}".`) +
    (p.message ? `\n\n"${plain(p.message, 300)}"` : '') +
    `\n\nSee the offer: ${link}\n\n` +
    `${HOLDS_NOTHING}\n\n` +
    `Stop emails about offers: ${unsub}\n`;

  return { subject, html, text };
}

/** Everything the mail path needs, or null with the reason logged once. */
function mailConfig(env) {
  const missing = ['SUPABASE_SECRET_KEY', 'RESEND_API_KEY', 'MAIL_FROM', 'UNSUB_SECRET']
    .filter((k) => !env[k]);
  if (missing.length) {
    // Deploys land before secrets do. Say which ones rather than failing in a
    // way that reads as a broken queue.
    console.log(`email: not configured, missing ${missing.join(', ')} -- nothing sent`);
    return null;
  }
  return {
    ...
    unsub: env.UNSUB_SECRET,
  };
}

/** Call one of the outbox RPCs. Returns null on anything unexpected. */
async function rpc(cfg, name, args) {
  ...
  if (res.status === 404) {
    // The migration is a file until somebody runs it. Until then this fires
    // every minute, so it has to say what is wrong rather than just failing.
    console.log(`email: ${name} does not exist -- apply 20260821_email_outbox.sql`);
    return null;
  }
  if (!res.ok) {
    console.log(`email: ${name} failed ${res.status} ${(await res.text()).slice(0, 200)}`);
    return null;
  }
 ...
 *
 * The idempotency key is the outbox row id, which is what makes the whole
 * retry story safe: a Worker that sends successfully and then dies before it
 * can record that will retry the same row, and Resend recognises it rather
 * than mailing the person twice.
 */
async function sendOne(cfg, row) {
  // to_profile is SET NULL when somebody deletes their account, which can
  // happen inside the five minutes this row was waiting. Sending anyway would
  // mail a closed account, and the unsubscribe link would carry no user to
  // unsubscribe. Retire it rather than retrying it five times.
  if (!row.to_profile) {
    await rpc(cfg, 'mark_email_sent', { p_id: row.id });
    return false;
  }

  const sig = await unsubSig(cfg.unsub, row.to_profile, row.kind);
  const unsub = unsubUrl(row.to_profile, row.kind, sig);
  const { subject, html, text } = offerEmail(row, unsub);
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      text,
      headers: {
        // Gmail requires one-click unsubscribe from anything sending at
        // volume, and honouring it is cheaper than being classified by not.
  ...
  await rpc(cfg, 'mark_email_failed', {
    p_id: row.id,
    p_error: `resend ${res.status}: ${(await res.text()).slice(0, 300)}`,
  });
  return false;
}

/**
 * Drain the outbox. Bounded at 20 a run, once a minute, which is far more
 * headroom than this site's volume needs and keeps one bad batch from
...
async function drainOutbox(env) {
  ...
  const rows = await rpc(cfg, 'claim_email_batch', { p_limit: 20 });
  if (!Array.isArray(rows) || !rows.length) return;

  let sent = 0;
  for (const row of rows) {
    try {
      if (await sendOne(cfg, row)) sent++;
    } catch (err) {
      // One bad row must not strand the rest of the batch.
      await rpc(cfg, 'mark_email_failed', { p_id: row.id, p_error: String(err).slice(0, 300) })
        .catch(() => {});
    }
  }
  console.log(`email: sent ${sent}/${rows.length}`);
}

/**
 * Unsubscribe.
 *
 * GET shows a button; POST is what actually turns the emails off. That split
 * is not politeness -- corporate mail scanners and link-preview fetchers open
 * every URL in a message, and a GET that unsubscribes would silently opt
 * people out of mail they never even read. One-click clients POST, so they
 * still work in a single step.
 */
async function unsubscribe(request, env) {
    ...
    new Response(
      `<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">` +
        ...
        `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;
           max-width:420px;margin:15vh auto;padding:0 24px;text-align:center;color:#18181b">${body}</div>`,
      { status, headers: { 'content-type': 'text/html; charset=utf-8' } }
    );

  const cfg = mailConfig(env);
  if (!cfg) return page('<p>Email settings are unavailable right now.</p>', 503);
  ...
  if (!sameSig(sig, await unsubSig(cfg.unsub, user, kind))) {
    return page('<p>That unsubscribe link is not valid.</p>', 400);
  }

  if (request.method !== 'POST') {
    return page(
      `<p style="font-size:17px">Stop emailing you about offers?</p>` +
        `<form method="POST"><button style="background:#18181b;color:#fff;border:0;
           padding:12px 20px;border-radius:8px;font-size:15px;font-weight:600;cursor:pointer">
           Yes, stop these emails</button></form>` +
        `<p style="font-size:13px;color:#71717a;margin-top:20px">
           You will still get emails about your account, like password resets.</p>`
    );
  }

  // `on=1` is how the undo below comes back through here. Anything else turns
  // the emails off, so a mangled link fails towards silence rather than
  // towards mail somebody has already said they do not want.
  const on = url.searchParams.get('on') === '1';

  const ok = await rpc(cfg, 'set_email_pref', { p_profile: user, p_kind: kind, p_on: on });
  if (ok === null) return page('<p>That did not save. Please try again.</p>', 502);

  if (on) return page('<p style="font-size:17px">Done — offer emails are back on.</p>');

  // The undo lives here because there is nowhere else for it to live: there is
  // no preferences screen yet, and telling somebody to go and find one would
  // be inventing a feature in a footer.
  const undo = `${url.pathname}?u=${encodeURIComponent(user)}&k=${encodeURIComponent(kind)}&s=${sig}&on=1`;
  return page(
    `<p style="font-size:17px">Done — no more offer emails.</p>` +
      `<form method="POST" action="${esc(undo)}">` +
      `<button style="background:none;border:0;color:#71717a;font-size:13px;text-decoration:underline;
         cursor:pointer;padding:0">Actually, turn them back on</button></form>`
  );
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Served by the Worker rather than added as an app route: it is reached
    // from an email by people who are not signed in and may not have the app
    // open, and a bracket route would be one more thing to teach the rewrite
    // below about. It also keeps the client bundle exactly where it was.
    if (url.pathname === '/api/unsubscribe') return unsubscribe(request, env);
  ...
  /**
   * The cron, once a minute. Everything it needs is in the outbox; if the
   * table, the secrets or both are missing it logs why and does nothing.
   *
   * waitUntil rather than a bare await so a slow Resend call cannot make the
   * scheduled event itself time out and be recorded as a failed run.
   */
